...
type SmartPhoneBrandsProps = {
  setSelectedComputerBrand: (brand: string) => void;
};

const ComputerBrands: React.FC<SmartPhoneBrandsProps> = ({ setSelectedComputerBrand }) => {
      ...
      <div className="my-4 grid grid-cols-2 gap-4 md:grid-cols-3">
        {brands.map((brand) => (
          <button
            key={brand.label}
            onClick={() => {
              setSelectedComputerBrand(brand.label);
            }}
