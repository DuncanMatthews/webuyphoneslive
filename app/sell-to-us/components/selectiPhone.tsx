...
const SellPhone: React.FC<SellPhoneProps> = ({
  selectediPhoneModel,
  setselectediPhoneModel,
  selectediPhoneColor,
  selectediPhoneCondition,
  selectediPhoneStorage,
  setselectediPhoneColor,
  setselectediPhoneStorage,
  setselectediPhoneCondition
}) => {
    ...
    <div className="min-h-screen">
      ...
      <div className="flex h-full flex-col items-center justify-center">
          ...
          <div className="flex flex-col items-center justify-center py-6 ">
            ...
            <div className="my-4 grid grid-cols-2 gap-4 md:grid-cols-4">
              {Object.keys(iPhoneDetails).map((model) => (
                <button
                  key={model}
                  onClick={() => setselectediPhoneModel(model as iPhoneModel)}
        ...
        !selectediPhoneStorage ? (
          <div className="flex flex-col items-center justify-center py-6 ">
            <h2 className="mb-4 py-6 text-xl font-semibold">
              Select storage for your {selectediPhoneModel}
            </h2>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {iPhoneDetails[selectediPhoneModel].storage.map((storage) => (
                <button
                  key={storage}
                  onClick={() => setselectediPhoneStorage(storage)}
        ...
        !selectediPhoneColor ? (
          <div className="flex flex-col items-center justify-center py-6 ">
            <h2 className="mb-4 py-6 text-xl font-semibold">
              Select color for your {selectediPhoneModel} {selectediPhoneStorage}
            </h2>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {iPhoneDetails[selectediPhoneModel].colors.map((color) => (
                <button
                  key={color}
                  onClick={() => setselectediPhoneColor(color)}
                  className="rounded-lg border p-4 shadow hover:bg-gray-200 hover:text-black focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
        ...
        !selectediPhoneCondition ? (
          <div className="flex flex-col items-center justify-center py-6 ">
            <h2 className="mb-4 py-6 text-xl font-semibold">
              What's the condition of your {selectediPhoneModel}?
            </h2>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {iPhoneConditions.map((condition) => (
                <button
                  key={condition}
                  onClick={() => setselectediPhoneCondition(condition)}
                  className="rounded-lg border p-4 shadow hover:bg-gray-200 hover:text-black focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
          ...
          <div className="bg-white-900 flex flex-col items-center p-8 shadow-md">
            ...
            <div className="rounded-lg bg-white p-8 shadow-xl">
              <p className="mb-4 text-gray-800">
                ...
                <span className="font-semibold">{selectediPhoneModel}</span>
              </p>
              <p className="mb-4 text-gray-800">
                <span className="font-medium text-gray-500">Storage:</span>
                <span className="font-semibold">{selectediPhoneStorage}</span>
              </p>
              <p className="mb-4 text-gray-800">
                <span className="font-medium text-gray-500">Color:</span>
                <span className="font-semibold">{selectediPhoneColor}</span>
              </p>
              <p className="text-gray-800">
                <span className="font-medium text-gray-500">Condition:</span>
                <span className="font-semibold">{selectediPhoneCondition}</span>
              </p>
            </div>
          </div>
        )}
